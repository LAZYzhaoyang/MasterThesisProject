...
from ..data_loader import NeighborsDataset
...
def contrastive_evaluate(val_loader, model, memory_bank):
    ...
    for batch in val_loader:
        ...
        targets = ToTensor(batch['label']).cuda(non_blocking=True)
        ...
        acc1 = 100*torch.mean(torch.eq(output, targets).float())
        top1.update(acc1.item(), nodes.size(0))

    return top1.avg


@torch.no_grad()
def get_predictions(cfg, dataloader, model, return_features=False):
    # Make predictions on a dataset with neighbors
    model.eval()
    predictions = [[] for _ in range(cfg.model_config['nheads'])]
    probs = [[] for _ in range(cfg.model_config['nheads'])]
    targets = []
    if return_features:
        ft_dim = cfg.model_config['feature_dim']
        features = torch.zeros((len(dataloader.sampler), ft_dim)).cuda()
    # judge it's the neighbors dataset or others 
    ...
    for batch in dataloader:
        node = ToTensor(batch[key_]).cuda(non_blocking=True)
        bs = node.shape[0]
        res = model(node, forward_pass='return_all')
        output = res['output']
        if return_features:
        ...
        if cfg.model_config['task'] == 'supervised':
            targets.append(ToTensor(np.argmax(batch['label'], axis=-1)).cuda(non_blocking=True))
        else:
            targets.append(ToTensor(batch['label']).cuda(non_blocking=True))
    ...
    if include_neighbors:
        ...
        out = [{'predictions': pred_, 'probabilities': prob_, 'label': targets, 'neighbors': neighbors} for pred_, prob_ in zip(predictions, probs)]

    else:
        out = [{'predictions': pred_, 'probabilities': prob_, 'label': targets} for pred_, prob_ in zip(predictions, probs)]
...
def hungarian_evaluate(subhead_index, all_predictions, class_names=None, 
    ...
    targets = ToTensor(head['label']).cuda()
    predictions = head['predictions'].cuda()
    probs = head['probabilities'].cuda()
    # number of pred and truth classes
    truth_num_classes = torch.unique(targets).numel()
    pred_num_classes = torch.unique(predictions).numel()
    num_classes = max(truth_num_classes, pred_num_classes)
    # number of elements
    num_elems = targets.size(0)
    # matching the predicts and targets
    ...
    # _, preds_top4 = probs.topk(4, 1, largest=True)
    # reordered_preds_top4 = torch.zeros_like(preds_top4)
    # for pred_i, target_i in match:
    #     reordered_preds_top4[preds_top4 == int(pred_i)] = int(target_i)
    # correct_top4_binary = reordered_preds_top4.eq(targets.view(-1,1).expand_as(reordered_preds_top4))
    # top4 = float(correct_top4_binary.sum()) / float(num_elems)

    # Compute confusion matrix
    
    confusion_matrixs = metrics.confusion_matrix(targets.cpu().numpy(), reordered_preds.cpu().numpy())
    ...
    #         'ACC Top-4': top4, 'hungarian_match': match, 
    #         'confusion_matrix':confusion_matrixs}
    return {'ACC': acc, 'ARI': ari, 'NMI': nmi, 
