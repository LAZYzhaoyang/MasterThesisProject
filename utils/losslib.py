...
def get_criterion(p):
    if p['criterion'] == 'simclr':   
        criterion = SimCLRLoss(temperature=p['temperature'])
...
class ResponseLoss(nn.Module):
    def __init__(self, node_weight=1, res_weight=1):
        super().__init__()
    ...
    def forward(self, pred_node, gt_node, pred_res, gt_res):
        loss1 = self.node_loss(pred_node, gt_node)
        loss2 = self.res_loss(pred_res, gt_res)
        loss = self.node_weight*loss1 + self.res_weight*loss2
...
class SimsiamLoss(nn.Module):
    ...
    def forward(self, pred1, pred2, proj1, proj2):
        loss = 1 - 0.5 * (self.criterion(pred1, proj2).mean() + self.criterion(pred2, proj1).mean())
...
class ByolLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.criterion = nn.CosineSimilarity(dim=-1)
    def forward(self, pred1, proj2):
        loss = 1 - self.criterion(pred1, proj2).mean()
